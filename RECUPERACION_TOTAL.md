...
## Objetivo
Recuperar **Método Exacto Coach** desde cero si se pierde el ordenador, la carpeta local o el entorno completo.
...
1. **Clonar el repo**
   ```bash
   git clone https://github.com/sadeg3n-ops/metodo-exacto-coach.git
   cd "metodo-exacto-coach"
   ```
...
8. **Comprobar el flujo del formulario / captación**
   - CTA principal: `app/page.tsx` intenta abrir Calendly con `NEXT_PUBLIC_CALENDLY_URL`.
   - Fallback: modal visual en `components/landing/cta-modal.tsx`.
   - WhatsApp: enlaces directos en FAQ y footer.
   - Hoy no hay envío server-side de datos desde el modal; es una UI de captación, no un backend de leads.
9. **Comprobar correo**
...
12. **Verificación final en producción**
    - Abrir https://metodo-exacto-coach.vercel.app
